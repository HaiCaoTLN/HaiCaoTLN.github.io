        ...
        await request(HTTP_GET, `/api/comment?per=${pagination.getPer()}&next=${pagination.getNext()}`)
            ...
            .then((res) => {
                ...
                if (res.data.length === 0) {
                    comments.innerHTML = `<div class="h6 text-center fw-bold p-4 my-3 bg-theme-${theme.isDarkMode('dark', 'light')} rounded-4 shadow">Let's spread the joy! Share this invitation and let the celebration begin with a cascade of comments.</div>`;
                    return;
                }
